...
pub fn handle_aes_ctr_command(
    ...
    let mut thread_quantity: usize ;


    for _j in 0..4{
		// block_len = 2048/hoch2(j+1); 
		let block_quant =
				if rest >= 1024*1024*1024{ 
						//~ für Gigabytes
							(8, 1073741824)
					}else if rest >= 1024*1024{
						//~ Für Megabytes
							(4, rest/4)
					}else{
						//~ Für kleine Dateien, keine Parallelisierung.
							(1, ((rest / 16) * 16))
						};
	thread_quantity = block_quant.0 as usize;
	block_len = block_quant.1 as u64;
	
		nbs = rest as u64 / block_len ;  // Anzahl der Duchlauf mit einem Datei von Größe block_len.
		...
		//~ for _ in 0..nbs{
			...
			//~ block_cipher(&t_data, &output_file_path, number_of_blocks, &sbox, &keys, &iv_bytes, nr, thread_quantity, block_len);
			//~ number_of_blocks += block_len/16;
		//~ }
		for _ in 0..nbs{
				//Lese Daten von input-file
				let t_data = reader(&input_file_path, number_of_blocks * 16, block_len as usize).ok().unwrap(); // number of blocks block_len/16
				// Konvertiere, sodass referenziert werden kann --> Speicheroptimierung
				let t_data = Arc::new(t_data);
				// Berechne AES-CTR
				block_cipher(t_data, &output_file_path, number_of_blocks, &sbox, &keys, &iv_bytes, nr, block_quant.0 as usize, block_len);
				number_of_blocks += block_len/16;
			}
		if rest ==0{break}
		if rest < 16{
...
fn block_cipher(t_data: Arc<Vec<u8>>, output_file_path: &PathBuf, index: u64, sbox: &Vec<u8>, 
			keys: &Vec<Vec<Vec<u8>>>, iv_bytes: &Vec<u8>, nr: usize, thread_quantity:usize, block_len:u64){
		
		
		let offset: usize = 16;
        let size:usize = block_len as usize / thread_quantity;	// Wie groß sollte der Block sein, der von einem Thread behandeln werden soll?.
        let factor:usize = size / offset; 							// Wie viele Blöcke sollte ein Thread behandeln?.
		let zeros: Vec<u8> = vec![0; size];
		let mut results:Vec<Vec<u8>> = vec![zeros.clone()];		// Initialisiere das Ergebnis Buffer mit Nullen, um die Parallesierung zu erleichtern.
		...
		for i in 0..thread_quantity  {
			...
			let t_data = Arc::clone(&t_data);
			let iv_bytes = iv_bytes.clone();
			//~ aes parallel ausführen
			let handle = thread::spawn(move || { 
                let mut result_teil:Vec<Vec<u8>> = Vec::new();
                let foo: usize = i*factor;
				for k in 0..factor{
					let mut input = to2d(&ctr128_inc(&iv_bytes, index + (foo +k) as u64));
					aes(&mut input, &keys, nr, &sbox);
					let cipher:Vec<u8> = input.clone().into_iter().flatten().collect();
					let inde:usize = (offset*(foo + k)) as usize;
					let pt:Vec<u8> = t_data[inde..inde + offset].to_vec();
...
fn block_cipher1(t_data: &Vec<u8>, output_file_path: &PathBuf, index: u64, sbox: &Vec<u8>, 
			keys: &Vec<Vec<Vec<u8>>>, iv_bytes: &Vec<u8>, nr: usize, thread_quantity:usize, block_len:u64){
