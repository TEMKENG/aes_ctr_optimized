...
fn get_file_as_byte_vec(filename: &String) -> Vec<u8> {
    ...
    println!("inputfile.len(): {:?}", buffer.len());
...
pub fn handle_aes_ctr_command(
    ...
    let blocks_bytes: Vec<u8> = get_file_as_byte_vec(&input_file_path.display().to_string());
    ...
    for i in 0..nbs {
        ...
        //~ println!("i= {}", i);
        let iv_in: Vec<u8> = ctr128_inc(&iv_bytes, i as u64);
